...
namespace ResourceManagment
{
	...
	Shader::Shader(std::string vertexShaderFileName, std::string fragmentShaderFileName)
	{
		...
		if (!linked)
		{
			...
			Core::Logging::logE("ERROR: Shader linking failed: " + std::string(log));
			delete[] log;
	...
	void Shader::initaliseShader(std::string shaderFileName, char shaderType)
	{
		...
		if (!CheckShaderCompiled(shader))
		{
			...
			Core::Logging::logE(shaderFileName + " Failed to compile.");
			//quit the function
			return;
		}
	...
	bool Shader::CheckShaderCompiled(GLint shader)
	{
		...
		if (!compiled)
		{
			...
			Core::Logging::logE("ERROR: Shader compilation failed: " + std::string(log));
			delete[] log;
