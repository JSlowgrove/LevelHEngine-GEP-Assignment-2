#include "PS_ParticleEffect3D.h"

/**************************************************************************************************************/

/*Constructs a ParticleEffect Object.*/
PS_ParticleEffect3D::PS_ParticleEffect3D(std::string objectName, std::unordered_map<std::string, RM_Object*> &objects,
...
PS_ParticleEffect3D::~PS_ParticleEffect3D()
{
	/*delete pointers*/
...
void PS_ParticleEffect3D::update(float dt)
{
...
void PS_ParticleEffect3D::draw(glm::mat4 &viewMatrix, glm::mat4 &projMatrix)
{
...
void PS_ParticleEffect3D::makeNewParticles(std::unordered_map<std::string, RM_Object*> &objects,
	std::unordered_map<std::string, RM_Shader*> &shaders)
{
	...
	if (emit)
	{
		...
		for (int i = 0; i < numberOfParticles; i++)
		{
			...
			particles.push_back(new PS_Particle3D
				(new RM_Model(vertexShaderName, fragmentShaderName, objectName, objects, shaders),
...
void PS_ParticleEffect3D::setEmitter(glm::vec3 emitter)
{
	/*set the emitter*/
	this->emitter = emitter;
}
...
void PS_ParticleEffect3D::setEmitting(bool emit)
{
	/*set the value of emit*/
	this->emit = emit;
}